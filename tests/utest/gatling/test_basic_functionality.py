...
from rmkbridge.gatling import GatlingHandler
from rmkbridge.errors import GatlingHandlerException
...
class GatlingBasicTests(TestCase):
    ...
    def setUp(self):
        self.handler = GatlingHandler(get_config()['rmkbridge.gatling'])

    def test_initialization(self):
        self.assertEqual(self.handler.keyword, 'run_gatling')
        self.assertEqual(self.handler._tags, ['GATLING'])

    @patch('rmkbridge.gatling.GatlingHandler._transform_tests')
    @patch('rmkbridge.gatling.validate_path')
    ...
    @patch('rmkbridge.gatling.GatlingHandler._report_rmkbridge_run')

...
from rmkbridge.errors import JUnitHandlerException, ResultFileIsNotAFileException
from rmkbridge.junit import JUnitHandler
...
class JUnitBasicTests(TestCase):
    ...
    def setUp(self):
        self.handler = JUnitHandler(get_config()['rmkbridge.junit'])

    def test_initialization(self):
        self.assertEqual(self.handler.keyword, 'run_junit')
        self.assertEqual(self.handler._tags, ['JUNIT', 'EXTRA_JUNIT_CASE'])

    @patch('rmkbridge.junit.JUnitXml')
    @patch('rmkbridge.junit.JUnitHandler._transform_tests')
    @patch('rmkbridge.junit.validate_path')
    ...
    @patch('rmkbridge.junit.JUnitHandler._report_rmkbridge_run')
                    ...
                    "suites": [
                        {
                            ...
                            "tests": [
                                {
                                    "name": "casea",
                                    "tags": ["rmkbridge-junit-unknown-execution-time"],
                                    "keywords": [
                                        {
